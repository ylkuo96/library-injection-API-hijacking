 ...
 *(v) fopen fclose fread fwrite fgetc fgets
...
__attribute__((constructor)) int start() {
    ...
    /*
    if(libc_fscanf == NULL){
        libc_fscanf = (int (*) (FILE *, const char *, ...))dlsym(handle, "__isoc99_fscanf");
    }
    if(libc_fprintf == NULL){
        libc_fprintf = (int (*) (FILE *, const char *, ...))dlsym(handle, "fprintf");
    }
    */
...
extern "C" ssize_t write(int fd, const void *buf, size_t count){
    ...
    fprintf(stderr, "write(\"%s\", %p, %zu) = %zd\n", target_path.c_str(), &buf, count, res);
...
extern "C" int __xstat(int __ver, const char *pathname, struct stat *statbuf){
    ...
    fprintf(stderr, "stat(\"%s\", %p {mode=%o, size=%jd}) = %d\n", pathname, &statbuf, Mode, Size, res);
...
extern "C" int __lxstat(int __ver, const char *pathname, struct stat *statbuf){
    ...
    mode_t Mode = statbuf->st_mode;
    off_t Size = statbuf->st_size;
    fprintf(stderr, "lstat(\"%s\", %p {mode=%o, size=%jd}) = %d\n", pathname, &statbuf, Mode, Size, res);
...
extern "C" FILE *fopen(const char *pathname, const char *mode){
    ...
    //cout << "fopen(\"" << pathname << "\", \"" << mode << "\") = " << res << endl;
...
extern "C" int fclose(FILE *stream){
    ...
    //cout << "fclose(\"" << target_path << "\") = " << res << endl;
...
extern "C" size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream){
    ...
    fprintf(stderr, "fwrite(%p, %zu, %zu, \"%s\") = %zu\n", &ptr, size, nmemb, target_path.c_str(), res);
...
extern "C" int __isoc99_fscanf(FILE *stream, const char *format, ...){
    ...
    va_end(args);
    int res = libc_fscanf(stream, format, ptr);

    int fd = stream->_fileno;
    string target_path = getfilename(fd);
    fprintf(stderr, "# fscanf(\"%s\", \"%s\", %p) = %d\n", target_path.c_str(), format, ptr, res); 

    fprintf(stderr, "%d %d", a, b);
    return res;
}

int ffprintf(stderr, FILE *stream, const char *format, ...){
    //int res = libc_fscanf(stream, format, ...);
    // return res;
    return 0;
}
*/
...
extern "C" ssize_t readlink(const char *pathname, char *buf, size_t bufsize){
    ...
    fprintf(stderr, "readlink(\"%s\", %p, %zu) = %zd\n", pathname, &buf, bufsize, res);
...
extern "C" size_t fwrite_unlocked(const void *ptr, size_t size, size_t n, FILE *stream){
    ...
    int fd = stream->_fileno;
    string target_path = getfilename(fd);
    fprintf(stderr, "fwrite_unlocked(%p, %zu, %zu, \"%s\") = %zu\n", &ptr, size, n, target_path.c_str(), res);
...
extern "C" int fputs_unlocked(const char *s, FILE *stream){
    ...
    int fd = stream->_fileno;
    string target_path = getfilename(fd);
    fprintf(stderr, "fputs_unlocked(%p, \"%s\") = %d\n", &s, target_path.c_str(), res);
...
extern "C" int fflush_unlocked(FILE *stream){
    ...
    int fd = stream->_fileno;
    string target_path = getfilename(fd);
    fprintf(stderr, "fflush_unlocked(\"%s\") = %d\n", target_path.c_str(), res);
